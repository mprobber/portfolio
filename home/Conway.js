...
export default class Conway extends Component {
  ...
  constructor(props) {
    super(props);
    const board = name;
    this.state = { board, width: 0, paused: true, pressed: false}
  }
  ...
  countNeighbors(x, y, board) {
    ...
    for (let i = x - 1; i < x + 2; i++) {
      for (let j = y - 1; j < y + 2; j++) {
        if (board.get(i) && board.get(i).has(j))
        count++;
      }
    }
    return count;
    ...
    for (let i = 0; i < height; i++) {
      ...
      for (let j = 0; j < width; j++) {
        row.push(
          <div
            key={j}
            className={ styles.cell + ' ' + (this.isFilled(i, j, board) ? styles.filled : '') }
            onClick={() => this.isFilled(i, j) ? this.clearCell(i, j) : this.fillCell(i, j)}
            onMouseOver={() => this.state.pressed && (!this.isFilled(i,j,board) ? this.fillCell(i, j) : this.emptyCell(i, j))}/>
        );
      }
      myBoard.push(
      ...
      <div>
        ...
        <div className={styles.pauseDiv}>
          <button onClick={() => this.setState({paused: !this.state.paused})}>
            <span className={this.state.paused ? styles.stopped : styles.playing}/>
          </button>
        </div>
      </div>
    );
